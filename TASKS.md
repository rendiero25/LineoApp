...
- [x] **P1-18 · The window nobody photographed** — `S` — `:feature:notepad`, `:feature:scientific`
  - A phone in landscape had no snapshot: every picture of these two screens was a portrait
    phone or a tablet, and both have height to spare. That is the window P1-17's defect lived
    in — the keypad's bottom row drawn below it, `0` unpressable — and no test could have
    caught it
  - `NotepadLandscapePaparazziTest` and `ScientificLandscapePaparazziTest`, one class each
    because layoutlib refuses two `Paparazzi` rules on one thread. Pixel 5 turned sideways,
    851 × 393 dp
  - The screen is padded by what the shell takes above it — the status bar and the 60 dp top
    bar. Without that the first take had the whole 393 dp, the chip strip fitted, and the
    picture showed the case that was never in doubt. `LineoAppShell` is `:app`'s and a feature
    may not reach for it, so the space it costs is a stated number
  - Two rows in the log closed rather than left open: P0-01 (Navigation Compose, answered by
    P1-16) and P0-11b-2 (the overflow button opens a menu now)
  - **DoD:** met. The notepad picture shows five rows and the fifth column; the scientific one
    shows the strip gone, the wide function rows gone, and the grid it scrolls
...
|---|---|---|---|
| 2026-08-25 | P0-01 | Navigation Compose or Navigation 3? Google now points multi-screen apps at Nav 3; `ARCHITECTURE.md` §1 assumes Navigation Compose | **Navigation Compose.** P1-16 migrated the shell's `when` over `rememberSaveable` flags to a `NavHost` with a serializable `Destination`, and it has since carried the module screens, history and settings on a device. Nav 3 stays a Phase 3 question — moving to it is one file, `MainActivity`, which is what P1-04-2 kept true on purpose |
...
| 2026-08-25 | P0-11b-2 | The overflow button opens nothing. History is P1-06, settings is P1-07, and there is no menu to show until one exists | Both exist now, so the button opens a `DropdownMenu`: history first, the visible modules in the middle, settings last, each with an icon and a divider between the sections. Its trailing edge is aligned to `KeypadEdge` rather than to the window, so the menu ends where the keypad's operator column does. **Closed** — the control does something |
...
| 2026-08-25 | P0-01 | Navigation Compose or Navigation 3 — the row above says "decide before P1-10 (app shell)", and P1-10 has now been done without touching navigation | Answered by P1-16, which is where a back stack was finally needed: opening a module from the menu and coming back. See the P0-01 row above for the decision itself |
...
| 2026-08-25 | P1-18 | The defect P1-17 fixed lived in a window no snapshot covered, and re-recording the existing images would not have shown it | Two classes, one per screen, on a Pixel 5 turned sideways. One class each because layoutlib refuses two `Paparazzi` rules on one thread — the same trade P1-09 recorded for the pseudo-locales |
| 2026-08-25 | P1-18 | The first take of the landscape snapshot was useless: with the whole 393 dp the chip strip fitted, so the picture showed the case that was never in doubt | The screen is padded by what the shell takes above it — the status bar and the 60 dp top bar, 112 dp on this device. `LineoAppShell` lives in `:app` and a feature may not reach for it (`AGENTS.md` §3), so the space it costs is stated as a number with the reason beside it. Caught by opening the picture rather than by trusting a green test, which is the same lesson as P1-15-0 |
| 2026-08-25 | P1-18 | `lint` reported five errors and so could not be a gate: four `AndroidGradlePluginVersion` notices and one unused `full_app_title` | AGP raised 9.3.1 → 9.3.2, a patch on the line already chosen (`AGENTS.md` §7 reserves *adding* a dependency; this is the same one). Verified by building both variants and re-running every check. `full_app_title` is the Play title of P1-11 and is deliberately drawn by nothing — it carries `tools:ignore="UnusedResources"` and a comment saying why, rather than being deleted and re-typed from a doc when the listing is filled in. `./gradlew lint` is now clean and can fail CI on something real |
