...
| open | P1-01 | Nothing maps `NotepadDocument` to the `documents` / `lines` rows of `docs/ARCHITECTURE.md` §6 yet, so a document cannot be loaded or saved | Rule 3: the mapping needs `:core:data` and `:feature:notepad` in one task. **Belongs with P1-03**, which owns the notepad screen and therefore the load-and-save path; P1-02 works on a document in memory and does not need it |
