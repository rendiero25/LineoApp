...
| open | P1-10-3 | Macrobenchmark refuses to run: "Android Studio Device Mirroring is active" — the phone is sending frames to a second display, which is exactly the kind of load that moves a startup number | Not suppressed, though the library offers `suppressErrors = "DEVICE-MIRRORING"` for it. The whole point of this task is a number that means something, and the flag would have produced a tidy measurement of the wrong device. No code change fixes it |
| open | P1-10-3 | Closing the mirroring session dropped the device off `adb` entirely, and `adb reconnect` could not recover it — the mirroring was what held the wireless debugging connection open | A circle: mirroring keeps the phone attached, and mirroring is what stops the benchmark. Two ways out, neither of them code — connect over **USB**, or connect wireless with `adb connect` directly so Android Studio never attaches and never starts mirroring. Worth unchecking *Settings → Tools → Device Mirroring → activate on connect* either way, so this does not recur |
| open | P1-10-3 | The wireless connection dropped twice in one session — once mid-run, taking the whole instrumented suite with it | Recorded because it decides how the numbers should be taken, not just that they were not: a Macrobenchmark run is 10–20 minutes and a drop loses all of it. USB also keeps the phone charging, and Macrobenchmark refuses on a low battery — it was at 48% when this was written |
