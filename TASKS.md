...
| 2026-08-23 | P0-14-2 | The result is rendered with `Quantity.canonicalString()`, which is locale-free. `docs/CONVENTIONS.md` §1 puts grouping and the locale separator at exactly this boundary, and no formatter exists yet | **Closed by P1-03-3.** `QuantityFormat` in `:core:ui` is that formatter, reaching every surface through `LocalQuantityFormat`, resolved once in `LineoAppShell` from the same settings that label the decimal key. No UI file references `canonicalString` any more — the notepad row, the converter, the tape and the editor all format through it, so the `id-ID` user sees `8.500.000`. Row found stale during P1-11 and closed against that evidence |
...
| 2026-08-23 | P1-03-3 | `QuantityFormatTest > Indian grouping is the platform's, not ours` fails: `hi-IN` groups as `1,234,567` on the JVM. `java.text.DecimalFormat` takes the *rightmost* grouping interval from a pattern, so `#,##,##0` gives it 3 and the lakh-crore grouping never happens — Android's ICU supports the secondary interval, the JVM the tests run on does not | **Closed.** The first of the two options was taken: the grouping is inserted rather than delegated, and the test that carried the platform's name in its title now reads `Indian grouping is lakh-crore, whatever the platform can express` — it asserts `12,34,567.89`, with a sibling for the crore pairs and one for a separator override. Not `@Ignore`d; it passes. Row found stale during P1-11 and closed against that evidence |
...
| 2026-08-23 | P1-06 | The history screen has no Paparazzi snapshot: `:app` does not apply the plugin, and adding it there is a build decision rather than a drive-by | **Closed**, exactly as this row predicted: the plugin was applied to `:app` when settings landed, and the history screen is snapshotted in light, dark and RTL, plus the expanded pseudo-locale and the maximum font scale. Test-only, so nothing reaches the APK. Row found stale during P1-11 |
...
| 2026-08-23 | P1-07 | The settings and licences screens have no Paparazzi snapshot, for the reason recorded against P1-06: `:app` does not apply the plugin | **Closed.** The plugin is applied and `:app` carries 18 snapshots — settings and licences each in light, dark and RTL, settings again with a comma separator chosen, and both in the expanded pseudo-locale, with settings at the maximum font scale. Row found stale during P1-11 |
